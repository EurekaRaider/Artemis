...
describe("Desktop semantic token snapshots", () => {
  it("requires every registered semantic custom property without inline fallback", () => {
    ...
    expect(Object.keys(complete ?? {})).toHaveLength(
      Object.keys(SEMANTIC_TOKEN_REGISTRY).length,
    );
