...
describe("release version", () => {
  it("keeps manifests, lockfile, MCP identity, theme, and README at v1.5.5", () => {
    ...
    expect(mcp.match(/version: "1\.5\.5"/gu)).toHaveLength(3);
    expect(
      themeArtemisSource.match(
        /ARTEMIS_THEME_VERSION = "([^"]+)" as const;/u,
      )?.[1],
    ).toBe(releaseVersion);
    expect(readme).toContain("The `1.5.5` packaging configuration produces:");
