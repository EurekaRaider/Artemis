...
describe("release version", () => {
  it("keeps manifests, lockfile, MCP identity, theme, and README at v1.4.61", () => {
    ...
    expect(mcp.match(/version: "1\.4\.61"/gu)).toHaveLength(3);
    expect(
      themeArtemisSource.match(
        /ARTEMIS_THEME_VERSION = "([^"]+)" as const;/u,
      )?.[1],
    ).toBe(releaseVersion);
    expect(readme).toContain("The `1.4.61` packaging configuration produces:");
