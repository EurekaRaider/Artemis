...
describe("release version", () => {
  it("keeps manifests, lockfile, MCP identity, theme, and README at v1.4.45", () => {
    ...
    expect(mcp.match(/version: "1\.4\.45"/gu)).toHaveLength(3);
    expect(
      themeArtemisSource.match(
        /ARTEMIS_THEME_VERSION = "([^"]+)" as const;/u,
      )?.[1],
    ).toBe(releaseVersion);
    expect(readme).toContain("The `1.4.45` packaging configuration produces:");
