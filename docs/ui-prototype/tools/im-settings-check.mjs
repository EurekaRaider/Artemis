...
try {
 ...
 await check('pairing approval and project save confirmation',async()=>{await locate('account');await page.locator('[data-copy-pair]').first().click();await page.locator('[data-im-pair-arrive]').click();await page.locator('[data-im-approve]').click();await locate('projects');await page.locator('[data-im-project-idx="0"] .im-project-head input').check();assert.equal(await page.locator('[data-im-default-project]').inputValue(),'Artemis');await page.locator('[data-im-projects-save]').click();assert.equal(await page.locator('.im-confirm-dialog').evaluate(el=>el.open),true);await page.locator('[data-im-confirm-ok]').click();await page.waitForTimeout(300);assert.equal(await page.locator('[data-im-test-done]').isHidden(),true);assert.equal(await page.locator('[data-im-card="test"] [data-im-badge]').textContent(),'当前步骤');});
