...
```sh
npm run build:core
npx vitest run apps/desktop/test/design-source.test.ts apps/desktop/test/design-store.test.ts packages/agent-host/test/design-workflow.test.ts
...
- Production preview manager with one-preview ownership, watchdog, RSS sampling, rate-limited bridge, all-frame navigation/download/permission adversarial tests, stale-instance rejection and preview stop on read-only mode changes. The probe currently tests immediate termination, not the full watchdog.
