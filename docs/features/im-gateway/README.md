...
2. 在第②步“添加机器人”配置机器人；企业微信填写企业 ID、Bot ID / Secret，飞书默认长连接，填写 App ID / Secret 并选择飞书／Lark 区域——Tenant Key 与 Bot Open ID 可留空，保存后自动获取；选择 HTTPS 回调时额外填写 Verification Token、Encrypt Key。外部管理凭据不会保存；内置服务自动管理凭据，无需填写。机器人密钥只送到 Gateway 并加密保存。Slack 只需 Bot User OAuth Token 和 App-Level Token；工作区、应用和机器人编号自动识别。
...
先让已配对的群管理员在目标群 @机器人发送 `/help`，再在桌面设置的「群协作」流程中输入管理凭据、点击「刷新群和成员」：发现的群与已配对成员会直接列入表单，勾选要连接的群（endpoints）与成员（participants）、为这组群命名后保存即可，无需手工拼装结构；`groups`、`identities` 和 `devices` 诊断信息仍可用于核对实际群 ID、稳定身份及设备 ID。发现群 ID 不会自动开启共享。进阶设置也可以直接编辑以下空间 JSON（表单与 JSON 内容等价）：
...
自动测试覆盖协议隔离、渠道验签与自有 @、重复消息、双设备模拟协作、审批一次生效、原生 macOS Seatbelt 文件与网络边界及显式产物发布。`verify:im` 使用独立临时数据目录启动真实 Electron，验证五步引导、注册、配对、授权、Pi 受限文件读取和任务回传；模型和 IM 渠道使用本地测试替身，不会连接个人账号。截图与结果默认写入 `artifacts/im`。
