...
2. 在第三步“连接机器人”配置机器人；企业微信填写企业 ID、Bot ID / Secret，飞书默认长连接，填写 Tenant Key、App ID / Secret、Bot Open ID 和飞书／Lark 区域；选择 HTTPS 回调时额外填写 Verification Token、Encrypt Key。外部管理凭据不会保存；内置服务自动管理凭据，无需填写。机器人密钥只送到 Gateway 并加密保存。Slack 只需 Bot User OAuth Token 和 App-Level Token；工作区、应用和机器人编号自动识别。
