...
### P8 ④卡行内即时生效 + 默认项目徽章化（2026-09-14）

- **外层「保存并启用」退役**：勾选/撤销项目、设默认、弹窗「确认设置」全部即时走 `performSaveAndEnable`（两阶段：保存→启用）；启用失败仍钉④卡给重试入口。行内操作 `advance:false` 不触发自动前进，且 pin 住④卡——否则「自动跟随第一个缺步卡」会在④完成的瞬间把正在操作的卡片收起（调试用例实测行直接消失）。
- **默认项目下拉退役**：默认以行内徽章「默认」呈现（临时会话行或项目行）；悬停项目行出现「设为默认」（`visibility` 占位稳定不跳动）；取消勾选默认项目时默认回落临时会话。默认语义：空/哨兵 = 临时会话（P7）。
- **弹窗关闭 = 放弃**：`closeGrantDialog` 把该项目授权草稿回退到已保存状态（Escape/背景/关闭按钮一致）；「确认设置」成功才关窗。
- **e2e 适配**：click() 助手等待 disabled 解除（行内保存期间按钮短暂禁用）；「关闭+保存并启用」两步改「确认设置」一步。
- 门禁：仓库根 1677 通过 + typecheck + 收敛绿；e2e 过全部改动点，仍止于 main 既有的 @ 成员选择器失败（:1671）。
